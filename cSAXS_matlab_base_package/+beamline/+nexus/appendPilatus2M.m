%APPENDPILATUS2M read cbf data and append to already existing file or
...
function s = appendPilatus2M(s)
import beamline.nexus.*

load_dir = fullfile(s.pilatus2m.path, s.load_dir);

if ~exist(load_dir, 'dir')
    if s.verbose_level > 3
        warning('Raw data path for Pilatus2M %s not found', load_dir)
    end
    return
end
...
prepareTargetDir(s);
if ~exist(s.targetFile, 'file') || ~io.HDF.hdf5_dset_exists(s.targetFile, 'pilatus_1', '/entry/instrument')
    ...
    if ~isempty(res)
        ...
        if ~s.debug
            ...
            [stat, out] = system([s.pilatus2m.converter ' -s ' load_dir ' -o ' utils.abspath(s.targetFile) ' -p /entry/instrument/pilatus_1']);
            if stat
                fprintf(out);
                error('Failed to append Pilatus2M.');
            end
            
            if s.pilatus2m.delete_raw_data
