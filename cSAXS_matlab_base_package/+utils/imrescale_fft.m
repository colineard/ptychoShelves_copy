...
% matrix of fourier transformation, fast only for small arrays. Currently
% not working for stacks of 2D images.
%
% Inputs:  
...
function img_rescale = imrescale_fft(img, scale)
    ...
    if any(isnan(scale))
        warning('Some values of ''scale'' are nan, returning original array without resizing')
        img_rescale = img;
        return
    end
