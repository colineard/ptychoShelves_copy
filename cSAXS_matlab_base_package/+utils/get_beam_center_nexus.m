...
function [ center_xy ] = get_beam_center_nexus(filename,det,varargin)
import io.nexus_read
...
if (nargin < 1)
    ...
    fprintf('''TestY'',<integer>                   check +/- this many pixel around the good guess, default in y is %d\n',...
        test_y);
    fprintf('''Ifsaxs'',<integer>                  if current configuration is SAXS or WAXS, default is %d (SAXS)\n',...
        Ifsaxs);
...
fprintf('loading %s\n',filename);
frame = nexus_read(filename,'filter',det, 'orientation', [0 0]);
if ~isempty(filename_valid_mask) && exist(filename_valid_mask)
    mask = io.HDF.hdf5_load(filename_valid_mask, '/entry/collection/mask');
...
hold off;
clf;
frame_plot = math.applyTransform(mean(frame.data,3), ...
    [frame.orientation.transpose frame.orientation.rot90], 'transpose_rot90');
guess_plot = math.convert_location('pos', [guess_y, guess_x], ...
...
% frame_plot(guess_plot(1),(guess_plot(2)-20):(guess_plot(2)+20)) = 1e6;
% frame_plot((guess_plot(1)-20):(guess_plot(1)+20),guess_plot(2)) = 1e6;
imagesc(log10(frame_plot));
line([guess_plot(2)-20 guess_plot(2)+20], [guess_plot(1) guess_plot(1)], 'Color', 'r')
line([guess_plot(2) guess_plot(2)], [guess_plot(1)-20 guess_plot(1)+20], 'Color', 'r')
axis xy;
axis equal;
axis tight
colormap(plotting.colormaps.viridis);
colorbar();
valmax = math.sp_quantile(log10(frame_plot), [0 1-1e-3],10);
caxis([-1 valmax(2)]);
xlim([max(1, guess_plot(2)-150) min(guess_plot(2)+150, size(frame_plot,2))]);
ylim([max(1, guess_plot(1)-150) min(guess_plot(1)+150, size(frame_plot,1))]);
title([ 'beam center guess marked at (' num2str(guess_plot(2),'%.0f') ...
    ',' num2str(guess_plot(1),'%.0f') ')' ]);
...
arg_prep_integ_masks{11} = 'Det';
arg_prep_integ_masks{12} = det;
arg_prep_integ_masks{13} = 'Ifsaxs';
arg_prep_integ_masks{14} = Ifsaxs;
...
% display the result
cen_print = math.convert_location('pos', [cen_y, cen_x], ...
    'framesz', size(frame.data(:,:,1)), 'vecIn', [0 0], ...
    'vecTypeIn', 'transpose_rot90', 'vecOut', ...
    [frame.orientation.transpose frame.orientation.rot90], ...
    'vecTypeOut', 'transpose_rot90');
fprintf('Minimum standard deviation position (Y/X) interpolated to (%.3f,%.3f)\n',...
    cen_print(1),cen_print(2));

% plot the standard deviation as a function of tested pixel coordinates
raw_start = math.convert_location('pos', [y(end) x(end)], ...
'framesz', size(frame.data(:,:,1)), 'vecIn', [0 0], ...
'vecTypeIn', 'transpose_rot90', 'vecOut', ...
[frame.orientation.transpose frame.orientation.rot90], ...
'vecTypeOut', 'transpose_rot90');
raw_end = math.convert_location('pos', [y(1) x(1)], ...
'framesz', size(frame.data(:,:,1)), 'vecIn', [0 0], ...
'vecTypeIn', 'transpose_rot90', 'vecOut', ...
[frame.orientation.transpose frame.orientation.rot90], ...
'vecTypeOut', 'transpose_rot90');
figure(fig_no +1);
surf([raw_start(2):raw_end(2)],[raw_start(1):raw_end(1)],std_val);
colormap(plotting.colormaps.viridis)
colorbar;
title( ['standard deviation of the radial integration, center = (' ...
    num2str(cen_print(2),'%.1f') ', ' num2str(cen_print(1),'%.1f') ')' ] );
xlabel('x [ pixel ]');
ylabel('y [ pixel ]');
...
import beamline.prep_integ_masks_nexus
std_val = zeros(1,ind_x_max);
arg_prep_integ_masks{end+1} = 'Data';
arg_prep_integ_masks{end+1} = frame;
for (ind_x = 1:ind_x_max)
    fprintf('%3d / %3d\n',(ind_y-1)*ind_x_max + ind_x,ind_total);
    [ integ_masks ] = ...
        prep_integ_masks_nexus( [], [x(ind_x) y(ind_y)], ...
