    ...
    if use_MEX && ~isa(full_array, 'gpuArray') && ~verLessThan('matlab', '9.4') && ~islogical(small_array)  % logical arrays not yet implemented
        ...
        try
            ...
            if any(strcmp(err.identifier, { 'MATLAB:UndefinedFunction','MATLAB:mex:ErrInvalidMEXFile'}))
                ...
                src_file = fullfile(path, 'private', 'get_from_3D_projection_mex.cpp');
                out_file = fullfile(path, 'private', 'get_from_3D_projection_mex');
                
                % 判断当前操作系统
                if ispc
                    % Windows 下 (MSVC) 不需要 -fopenmp
                    mex('-R2018a','-O', src_file, '-output', out_file);
                else
                    % Linux / macOS 下 (g++)
                    mex('-R2018a','-O', ...
