...
function [entry] = struc2cell(data)


if ~isstruct(data)
    error('Expected structure as input data.')
else
    [entry] = rec_call(data);
end
    ...
    while true
        try
            if ~isstruct(data.(fn{ii}))
                ...
                entry = rec_call(data.(fn{ii}));
                for jj=1:length(entry)
                    en{end+1} = [fn{ii} '.' entry{jj}];
                end
                break;
            end
        catch
