% near_field_probe_curvature used to remove the curvature of model probe in
% near field regime
%
% [dH] = near_field_probe_curvature(Npix, lambda, extent)
% 
% Inputs:
%  ** Npix          - Number of pixels [Nx,Ny] of the propagated window
%  ** lambda        - wavelength
%  ** extent        - lateral size [dx,dy] of the window'
%
% *returns*
%  ++ dH            - gradient of the probe curvature
%
%
...
function [dH] = near_field_probe_curvature(Npix, lambda, extent)


    extent = extent(:)' .* ones(1,2);


    xgrid = (0.5+(-Npix(1)/2:Npix(1)/2-1))*extent(1)/Npix(1);
    ygrid = (0.5+(-Npix(2)/2:Npix(2)/2-1))*extent(2)/Npix(2);
