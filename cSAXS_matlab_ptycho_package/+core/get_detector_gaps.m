%GET_DETECTOR_GAPS extract detector gaps from a mask
%[modulearray, nummody, nummodx, fmaskdet] = get_detector_gaps(fmask)
%
% ** fmask          2D or 3D binary mask
%
% returns:
% ++ modulearray    N x 4 array with detector gaps locations (numrows,numcols, startY, startX)
% ++ nummody        number of modules in Y
% ++ nummodx        number of modules in X
% ++ fmaskdet       extracted mask which only contains the gaps
%
% see also: core.prep_h5data
...
function [modulearray, nummody, nummodx, fmaskdet] = get_detector_gaps(fmask)

asize = size(fmask(:,:,1));

fmaski = fmask(:,:,1); % First mask to detect modules (gaps)
...
if auxmodyo(end)
    indendmody = [indendmody asize(1)];
end

modulearray = zeros(nummody*nummodx,4);
fmaskdet = zeros(asize); % Module mask for current detector position
counter = 0;
for kk = 1:nummody
    for jj = 1:nummodx
        counter = counter+1;
