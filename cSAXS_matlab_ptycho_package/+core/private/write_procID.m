...
try
    if ~exist(fullfile(p.ptycho_package_path,  'utils', '.tmp_procID'), 'dir')
        mkdir(fullfile(p.ptycho_package_path,  'utils', '.tmp_procID'))
    end
    if ispc
    ...
    f = fopen(fullfile(p.ptycho_package_path,  'utils', '.tmp_procID', ['proc_' num2str(feature('getpid')) '.dat']), 'w');
