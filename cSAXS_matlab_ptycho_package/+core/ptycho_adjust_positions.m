...
if isfield(p, 'spec') && (check_option(p.spec.motor,'coarse_motors') && length(p.spec.motor.coarse_motors) == 2)
    ...
    for ii = 1:length(p.scan_number)
        p.positions_real(p.scanidxs{ii},1) = p.positions_real(p.scanidxs{ii},1)-getfield(p.meta{ii}.specES1,p.spec.motor.coarse_motors{2})*p.spec.motor.coarse_motors_scale(min(end,2));
        p.positions_real(p.scanidxs{ii},2) = p.positions_real(p.scanidxs{ii},2)-getfield(p.meta{ii}.specES1,p.spec.motor.coarse_motors{1})*p.spec.motor.coarse_motors_scale(1);
    end
end
