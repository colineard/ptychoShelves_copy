...
function mask_proj = get_detector_projections(p, par, mask_static)
import utils.verbose
    
    detParams = p.detectors(p.scanID).params;

    p.det_pos = p.det_pos *1e-3;
    p.det_pos = p.det_pos./p.ds-min(p.det_pos./p.ds);
    object_size = detParams.geometry.sz + max(round(p.det_pos),[],1);
    
    Npos = length(p.scanidxs{p.scanID});
    if nargin < 4
        mask_proj = zeros([detParams.geometry.sz, Npos], 'like', mask_static);
    end
    if any(max(round(p.det_pos(p.scanidxs{p.scanID},:)))+detParams.geometry.sz > [size(mask_static,1),size(mask_static,2)])
        error('Object is too small for given positions')
    end
    
    if ~verLessThan('matlab', '9.4')
        % faster MEX based function
        positions = int32(p.det_pos(p.scanidxs{p.scanID},:));
        indices = int32(1:Npos);
        mask_proj = utils.get_from_3D_projection(mask_proj,mask_static,positions,indices);
    ...
    for jj = p.scanidxs{scan_id}
        Indy = round(p.positions(jj,1)) + (1:detParams.geometry.sz(1));
        Indx = round(p.positions(jj,2)) + (1:detParams.geometry.sz(2));
        mask_proj(:,:,jj-id_0,:) = object(Indy,Indx,:);
    end
