...
function object = set_detector_projections(data, det_positions, par, p)
import utils.verbose
    detParams = p.detectors(p.scanID).params;

    det_positions = det_positions *1e-3;
    det_positions = det_positions./p.ds-min(det_positions./p.ds);
    object_size = detParams.geometry.sz + max(round(det_positions),[],1);
    
    object = zeros(object_size, 'single');
    
    par.det_positions = det_positions(p.scanidxs{p.scanID},:);
    Npos = length(p.scanidxs{p.scanID});
    Nmodes =  size(object,4);
    if any(max(round(det_positions))+detParams.geometry.sz > size(object))
        error('Object is too small for given positions')
    end
    
    if Nmodes == 1
        % faster MEX based function
        positions = int32(det_positions);
        indices = int32(1:Npos);
        object = utils.add_to_3D_projection(data,object,positions,indices, true);
    else
        verbose(3, 'Using slow nonMEX version of set_projections')
        id_0 = p.scanidxs{scan_id}(1)-1;
        for jj = p.scanidxs{scan_id}
            Indy = round(p.det_pos(jj,1)) + (1:detParams.geometry.sz(1));
            Indx = round(p.det_pos(jj,2)) + (1:detParams.geometry.sz(2));
