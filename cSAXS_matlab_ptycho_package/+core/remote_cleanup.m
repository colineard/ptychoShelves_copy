...
if ~completed
    ...
    for ii=1:numel(p.scan_number)
        [status , ~] = system(['mv ' fullfile(p.queue.tmp_dir_remote, 'analysis', utils.compile_x12sa_dirname(p.scan_number(ii)), '*') ' ' p.save_path{ii}]);
        if status
            error(['Could not move reconstruction: mv ' fullfile(p.queue.tmp_dir_remote, 'analysis', utils.compile_x12sa_dirname(p.scan_number(ii)), '*') ' ' p.save_path{ii}]);
        end
    end
