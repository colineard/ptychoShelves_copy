...
if exist(fullfile(p.ptycho_package_path, '+detector', ['+' p.detector.name], [func ext]), 'file')
    p.detectors(p.detID).funcs.(func) = str2func(['detector.' p.detector.name '.' func]);
else
    if exist(fullfile(p.ptycho_package_path, '+detector', '+prep_data', [func ext]), 'file')
