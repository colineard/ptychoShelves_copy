...
%% load mask
if isfield(p.detectors(p.scanID).params, 'mask') && ~isempty(p.detectors(p.scanID).params.mask)
    [~, ~, ext] = fileparts(p.detectors(p.scanID).params.mask);
    switch ext
        case '.h5'
            % HDF5 mask files are in raw data orientation
            mask = io.HDF.hdf5_load(p.detectors(p.scanID).params.mask, p.detectors(p.scanID).params.maskPath);
            detStorage.mask = logical(math.applyTransform(mask, p.detectors(p.scanID).params.orientation, 'transpose_fliplr_flipud'));
        case '.mat'
            % mat files are assumed to be in user orientation (legacy)
            m = load(p.detectors(p.scanID).params.mask);
            detStorage.mask = logical(m.mask);
        otherwise
            error('Unknown file extension');
    end

else 
    detStorage.mask = ones(size(p.detectors(p.scanID).detStorage.data(:,:,1)), 'logical');
end


%% crop mask to readout geometry (if necessary)

if isfield(p.detectors(p.scanID).params, 'geometry') && ~isempty(p.detectors(p.scanID).params.geometry.mask)
    detStorage.mask = detStorage.mask(p.detectors(ii).params.readout{1},p.detectors(ii).params.readout{2},:);
end
...
%% (Fourier ptycho specific options)
if isfield(detParams, 'filt_pinhole') && ~isempty(detParams.filt_pinhole)
    detStorage.fmask = detStorage.fmask.*utils.filt2d_pad(size(detStorage.fmask(:,:,1),1),round(detParams.filt_pinhole*0.9),round(detParams.filt_pinhole*0.9), 'circ');
end
if ~isempty(detStorage.staticMask)
    detStorage.fmask = detStorage.fmask .* math.fftshift_2D(detStorage.staticMask);
end
