...
if (isfield(p.detector,'binning')&& p.detector.binning) || (isfield(p.detector,'upsampling')&& p.detector.upsampling)|| (isfield(p.detector,'interpolate')&& p.detector.interpolate)
    p = core.apply_binning(p, 2^(p.detector.binning - p.detector.upsampling - p.detector.interpolate));   % modify the p structure after binning 
end
