...
if isfield(p, 'split_scan') && ~isempty(p.split_scan)
    if p.scanID > 1
        error('Splitting a scan is currently supported for one scan only.')
    end
    
    p.positions = p.positions(p.split_scan(1):p.split_scan(2):end,:);    
    p.positions_orig = p.positions_orig(p.split_scan(1):p.split_scan(2):end,:);
    p.positions_real = p.positions_real(p.split_scan(1):p.split_scan(2):end,:);
    p.numpts = length(p.positions);
    ...
    detStorage.data = detStorage.data(:,:,p.split_scan(1):p.split_scan(2):end);
    detStorage.mask = detStorage.mask(:,:,p.split_scan(1):p.split_scan(2):end);
    detStorage.files = detStorage.files(p.split_scan(1):p.split_scan(2):end);
    detStorage.staticMask = detStorage.staticMask(:,:,p.split_scan(1):p.split_scan(2):end);
end
