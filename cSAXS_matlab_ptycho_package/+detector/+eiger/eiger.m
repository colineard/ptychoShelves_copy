...
det.mask = fullfile(p.base_package_path, 'detector_masks', 'eiger_3_valid_mask.h5');    % detector mask
...
if exist(fullfile(p.raw_data_path{1}, 'data'), 'dir')
    ...
    det.basepath_dir = 'eiger/';        % raw data directory, legacy, remove in future 
end
...
if exist(fullfile(p.raw_data_path{1}, 'data'), 'dir') 
    % take nexus internal structure
    det.H5_detector_path = '/entry/instrument/eiger_3/data/';
else
    % old eiger internal structure
    det.H5_detector_path = '/eh5/images';
end
...
% Uncomment all this when using Ian's Eiger raw data (without nexus) 
