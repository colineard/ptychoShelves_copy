...
p.  verbose_level = 2;
p.  base_path = '';                                 % Base path for the remote reconstruction.
p.  ptycho_package_path = '';                        % PtychoShelves path for the remote reconstruction. 
p.  base_package_path = '';                         % Base package path for the remote reconstruction.
p.  queue.remote_path = '';                         % Path to the remote queue. Needs to be accessible from (local) primary and (remote) replica. 
p.  prepare_data_path = '';                         % Data preparation path.
p.  queue.recon_latest_first = true;                      % Reconstruct latest dataset first. 
...
addpath(genpath(p.ptycho_package_path));
addpath(genpath(p.base_package_path));
