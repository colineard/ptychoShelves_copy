        ...
        while ischar(tline)
            ...
            if numel(str_parts)>1
                ...
                if strcmpi(fname(1:2), 'p.')
                    ...
                    pos_colon= strfind(val,':');
                    if (~isempty(pos_colon)&&(~isnan(str2double(val(1:pos_colon-1)))))
                        p_out.(fname(3:end)) = (str2double(val(1:pos_colon-1)):str2double(val(pos_colon+1:end)));
                    else
