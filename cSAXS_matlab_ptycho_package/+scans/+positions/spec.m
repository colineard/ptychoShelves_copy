...
if ~isfield(p.meta, 'specES1')
    spec_aux = spec_read(p.specfile,'ScanNr',p.scan_number);
    if iscell(spec_aux)
        for ii=1:numel(p.scan_number)
            p.meta{ii}.specES1 = spec_aux{ii};
        end
    else
        p.meta{1}.specES1 = spec_aux;
    end
end
...
for ii = 1:length(p.scan_number)
    ...
    if p.scan.is_cont % If its a continuous scan and spec coordinates were requested (checked above)
        ...
        positions_real = [p.meta{ii}.specES1.(p.spec.motor.fine_motors{1})*p.spec.motor.fine_motors_scale(1) p.meta{ii}.specES1.(p.spec.motor.fine_motors{2})*p.spec.motor.fine_motors_scale(2)];
