...
if ~isfield(p.meta{1}, 'specES1')
    for ii=1:numel(p.scan_number)
        p.nexusfile{ii} = fullfile(p.raw_data_path{1}, p.nexus_directory, utils.compile_x12sa_dirname(p.scan_number(ii)), sprintf([p.detector.data_prefix '*%05d*'], p.scan_number(ii)));
        res = dir(p.nexusfile{ii});
        while isempty(res)
            res = dir(p.nexusfile{ii});
            verbose(1, 'Waiting for nexus file %s of scan %05d.', p.nexusfile{ii}, p.scan_number(ii));
            pause(0.5)
        end
        if length(res)>1
            error('Found more than one nexus file in %s', p.nexusfile{ii});
        else
            p.nexusfile{ii} = fullfile(res(1).folder, res(1).name);
            while ~utils.nexus_file_is_ready(p.nexusfile{ii})
                verbose(1, 'Waiting for nexus file of scan %05d.', p.scan_number(ii));
                pause(0.5)
            end
        end
    end
...
if isempty(p.meta{1})
for ii=1:numel(p.scan_number)
    p.meta{ii} = io.HDF.hdf5_load(p.nexusfile{ii}, '/entry/collection/');
end
end
...
if (isfield(p.spec,'isptycho')&&(numel(p.spec.isptycho)>0))
    ...
    for jj = 1:numel(p.spec.isptycho)
        if ~isempty(findstr(p.meta.specES1{jj}.S,p.spec.isptycho{jj}))
            isptycho = 1;
        end
    end
    if ~isptycho
